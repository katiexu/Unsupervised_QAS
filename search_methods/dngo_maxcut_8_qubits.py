...
def get_features(graph, dataset, index, features, energy, training_time):
    circuit_list = dataset[index]['op_list']
    obj, time_cost, _ = vqe_maxcut(graph, circuit_list, sample=False)
...
def get_init_samples(graph, dataset, features, energy, training_time, visited):
    ...
    for i in range(len(init_inds)):
        ...
        _, init_feat_samples[i], init_energy_samples[i], init_time_samples[i] = \
            get_features(graph, dataset, idx, features, energy, training_time)
    return init_inds, init_feat_samples, init_energy_samples, init_time_samples, visited


def propose_location(ei, graph, dataset, features, energy, training_time, visited):
    k = args.topk
...
def expected_improvement_search(args, graph, dataset):
    ...
    index_samples, feat_samples, energy_samples, time_samples, visited = \
        get_init_samples(graph, dataset, features, energy, training_time, visited)
    ...
    while counter < MAX_SAMPLE:
        ...
        index_next, feat_next, energy_next, time_next, visited = \
            propose_location(ei, graph, dataset, features, energy, training_time, visited)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_path', type=str, default='saved_logs\\bo\\maxcut', help='rl, rs or bo; fidelity, maxcut or vqe')
    parser.add_argument('--emb_path', type=str, default='maxcut-model-circuits_8_qubits_20_gates.pt')
    parser.add_argument("--threshold", type=float, default=0.925, help="maxcut threshold (default 0.925)")
    parser.add_argument("--objective", type=float, default=0.9, help="expected objective optimization (default 0.9)")
    ...
    expected_improvement_search(args, graph, dataset)