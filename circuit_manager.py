...
def translator(self, selected_single, selected_enta):
    ...
    for layer in range(self.num_layers):
        if layer in single_columns:
            circuit_ops.extend(non_empty_single_columns[layer])
        if layer in enta_columns:
            circuit_ops.extend(non_empty_enta_columns[layer])
...
if __name__ == '__main__':
    ...
    print("The first curcuit list: ", circuits[1])
    ...
    print("gate_matrix\n", gate_matrix)
    print("adj_matrx: \n", adj_matrix)
    fig, ax = qml.draw_mpl(circuit_qnode)(circuits[1])
    plt.show()
