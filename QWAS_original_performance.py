...
def Scheme(design, task, weight='base', epochs=None, verbs=None, save=None):
    ...
    for epoch in range(epochs):
        ...
        if epoch == epochs - 1:
            if not os.path.isfile('results_for_analysis.csv'):
                with open('results_for_analysis.csv', 'w', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow(
                        ['design', 'alpha', 'epoch', 'train_loss', 'train_acc', 'val_acc', 'test_acc', 'best_test_acc'])
            new_row = [design, 'original', epoch, train_loss[0], train_loss[1], val_loss_list[-1], metrics, best_test_acc]
            with open('results_for_analysis.csv', 'a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow(new_row)
...
if __name__ == '__main__':
    ...
    for i in range(len(selected_circuits[0]['op_list'])):
        ...
        elif selected_circuits[0]['op_list'][i][0] == 'RX':
            design.append(('U3', [selected_circuits[0]['op_list'][i][1]]))
        elif selected_circuits[0]['op_list'][i][0] == 'RY':
            design.append(('data', [selected_circuits[0]['op_list'][i][1]]))
        elif selected_circuits[0]['op_list'][i][0] == 'RZ':
            design.append(('data', [selected_circuits[0]['op_list'][i][1]]))
            design.append(('U3', [selected_circuits[0]['op_list'][i][1]]))
        else:
