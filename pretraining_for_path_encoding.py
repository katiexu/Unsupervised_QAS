...
from model_for_path_encoding import VAEReconstructed_Loss, GVAE
from utils.utils import load_json, save_checkpoint_vae_path_encoding, preprocessing
from utils.utils import get_val_acc_vae_path_encoding, is_valid_circuit
...
def pretraining_model(dataset, cfg, args):
    ...
    for epoch in range(0, epochs):
        ...
        for i, (adj, ops, ind) in enumerate(zip(X_adj_split, X_ops_split, indices_split)):
            ...
            # forward
            ops_recon, adj_recon, mu, logvar, adjusted_ops_recon, adjusted_adj_recon = model(ops, adj)
            Z.append(mu)
        ...
        for _ in range(args.latent_points):
            ...
            z = torch.randn(Z.shape[1], args.dim).cuda()
            z = z * z_std + z_mean
            # if epoch == args.epochs - 1:
            #     torch.save(z, 'z.pt')
            full_op, full_ad, full_adjusted_ops_recon, full_adjusted_adj_recon = model.decoder(z.unsqueeze(0))
            full_op = full_adjusted_ops_recon.squeeze(0).cpu()
            ad = full_adjusted_adj_recon.squeeze(0).cpu()
        ...
        acc_ops_val, mean_corr_adj_val, mean_fal_pos_adj_val, acc_adj_val = get_val_acc_vae_path_encoding(model, cfg, X_adj_val, X_ops_val, indices_val)
        ...
        save_checkpoint_vae_path_encoding(model, optimizer, epoch, sum(loss_epoch) / len(loss_epoch), args.dim, args.name, args.dropout, args.seed)
