...
# plot 4-qubits relevant reward data of maxcut experiments
def plot_maxcut_rewards(dim, num_sample):
    # read REINFORCE data
    rl_all_avg_reward_per_100 = []
    rl_all_regret_energy = []
    rl_final_avg_reward = []
    for seed in range(1, 51):
        f_name = 'saved_logs\\rl\\maxcut\\dim{}\\run_{}_maxcut-model-circuits_4_qubits.json'.format(dim, seed)
    ...
    bo_final_avg_reward = []
    for seed in range(1, 51):
        f_name = 'saved_logs\\bo\\maxcut\\dim{}\\run_{}_maxcut-model-circuits_4_qubits.json'.format(dim, seed)
        if not os.path.exists(f_name):
            continue
        f = open(f_name)
        bo_data = json.load(f)
    ...
    rs_final_avg_reward = []
    for seed in range(1, 51):
        f_name = 'saved_logs\\rs\\maxcut\\run_{}_maxcut-rs-circuits_4_qubits.json'.format(seed)
        if not os.path.exists(f_name):
            continue
        f = open(f_name)
        rs_data = json.load(f)
    ...
    rl_avg_regret_energy = np.insert(rl_avg_regret_energy, 0, 10., axis=0)
    rl_avg_reward_per_100 = np.insert(rl_avg_reward_per_100, 0, 0., axis=0)
    bo_avg_regret_energy = np.insert(bo_avg_regret_energy, 0, 10., axis=0)
    bo_avg_reward_per_100 = np.insert(bo_avg_reward_per_100, 0, 0., axis=0)
    rs_avg_regret_energy = np.insert(rs_avg_regret_energy, 0, 10., axis=0)
    ...
    plt.title(f"4-qubit maxcut with {num_sample} searches")
    ...
    plt.savefig(os.path.join(args.save_path, "data", "4-qubits-maxcut_avg_reward_per_100.png"), dpi=400)
    plt.close()
    ...
    plt.legend(["Random Search", "REINFORCE", "Bayersian Optimization"], loc="lower right", fontsize=16)
    plt.savefig(os.path.join(args.save_path, "data", "4-qubits-maxcut_avg_reward_per_100_with_full_filling.png"), dpi=400)
    plt.close()
    ...
    plt.legend(["Random Search", "REINFORCE", "Bayersian Optimization"], loc="lower right", fontsize=16)
    plt.savefig(os.path.join(args.save_path, "data", "4-qubits-maxcut_avg_reward_per_100_with_var_filling.png"), dpi=400)
    plt.close()

    # plot regret energy from 0
    fig = plt.figure(figsize=(8, 8))
    plt.xticks(np.arange(0, num_sample + 1, 100, dtype=int))
    plt.yticks(np.arange(0, 10.1, 1, dtype=float))
    plt.xlabel("search steps", fontsize=18)
    ...
    plt.savefig(os.path.join(args.save_path, "data", "4-qubits-maxcut_regret_energy_from_0.png"), dpi=400)
    plt.close()

    # plot regret energy from 0 to 100
    fig = plt.figure(figsize=(8, 8))
    plt.xticks(np.arange(0, 101, 10, dtype=int))
    plt.yticks(np.arange(0, 10.01, 1, dtype=float))
    plt.xlabel("search steps", fontsize=18)
    plt.ylabel("regret energy", fontsize=18)
    plt.title(f"4-qubit maxcut with {num_sample} searches")
    ...
    plt.legend(["Random Search", "REINFORCE", "Bayersian Optimization"], fontsize=16)
    plt.savefig(os.path.join(args.save_path, "data", "4-qubits-maxcut_regret_energy_from_0_to_100.png"), dpi=400)
    plt.close()
    ...
    plt.legend(["Random Search", "REINFORCE", "Bayersian Optimization"], fontsize=16)
    plt.savefig(os.path.join(args.save_path, "data", "4-qubits-maxcut_regret_energy_from_100.png"), dpi=400)
    plt.close()
    ...
    plt.legend(["Random Search", "REINFORCE", "Bayersian Optimization"], fontsize=16)
    plt.savefig(os.path.join(args.save_path, "data", "4-qubits-maxcut_regret_energy_from_100_with_var_willing.png"), dpi=400)
    plt.close()
    ...
    plt.legend(["Random Search", "REINFORCE", "Bayersian Optimization"], fontsize=16)
    plt.savefig(os.path.join(args.save_path, "data", "4-qubits-maxcut_regret_energy_from_200.png"), dpi=400)
    plt.close()
    ...
    plt.legend(["Random Search", "REINFORCE", "Bayersian Optimization"], fontsize=16)
    plt.savefig(os.path.join(args.save_path, "data", "4-qubits-maxcut_regret_energy_from_300.png"), dpi=400)
    plt.close()

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="4-qubits-maxcut_experiments_reward")
    ...
    plot_maxcut_rewards(args.dim, args.num_sample)